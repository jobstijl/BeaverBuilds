...
| `Dep::resource_value(\                                               | r: &R\                                                                                                                                 | …)`, `this_value`, `entity_value`, `parent_value`, `ancestor_value` | a *projection* of the data changed value — per-field wake granularity, tick-gated (the `ancestor` form re-walks each check; its projection is still tick-gated) |
